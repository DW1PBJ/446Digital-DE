...
# Generate Host Files
curl --fail -o ${APRSHOSTS} -s https://raw.githubusercontent.com/446Digital/446Digital-DE/main/APRSHosts.txt --user-agent "Pi-Star_${pistarCurVersion}"
curl --fail -o ${DCSHOSTS} -s http://www.pistar.uk/downloads/DCS_Hosts.txt --user-agent "Pi-Star_${pistarCurVersion}"
curl --fail -o ${DMRHOSTS} -s https://raw.githubusercontent.com/446Digital/446Digital-DE/main/DMR_Hosts.txt --user-agent "Pi-Star_${pistarCurVersion}"
...
curl --fail -o ${DMRIDFILE} -s  https://raw.githubusercontent.com/446Digital/446Digital-DE/main/DMRIds.dat --user-agent "Pi-Star_${pistarCurVersion}"
...
curl --fail -o ${TGLISTBM} -s https://raw.githubusercontent.com/446Digital/446Digital-DE/main/TGList_BM.txt --user-agent "Pi-Star_${pistarCurVersion}"
...
if [ -f ${NEXTIONGROUPS} ]; then
	# Update ${NEXTIONGROUPS}
 	if [[ $(find "${NEXTIONGROUPS}" -mtime +7) ]]; then
  		curl --fail -o ${NEXTIONGROUPS} -s https://raw.githubusercontent.com/446Digital/446Digital-DE/main/wpsd_groups.txt --user-agent "Pi-Star_${pistarCurVersion}"
  	fi
else
	# Get ${NEXTIONGROUPS}
 	curl --fail -o ${NEXTIONGROUPS} -s https://raw.githubusercontent.com/446Digital/446Digital-DE/main/wpsd_groups.txt --user-agent "Pi-Star_${pistarCurVersion}"
...
cp /root/HostFilesUpdate.sh /usr/local/sbin/HostFilesUpdate.sh
chmod 777 -R /root/
